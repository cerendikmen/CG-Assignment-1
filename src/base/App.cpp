...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
		...
		v0.position = { 0, 0, 0 };
		v1.position = { FW::cos(angle_increment * i) * radius, -height, FW::sin(angle_increment * i) * radius };
		v2.position = { FW::cos(angle_increment * (i + 1)) * radius, -height, FW::sin(angle_increment * (i + 1)) * radius };

		// v0.position = ...
		// Calculate the normal of the face from the positions and use it for all vertices.
		// v0.normal = v1.normal = v2.normal = ...;
		//
		v0.normal = v1.normal = v2.normal = (cross(v1.position, v2.position).normalized());
...
void App::render() {
	...
	Mat4f modelToWorld;
	Mat4f translation(Mat4f::translate(v));
